...
app.all('/twiml/:sessionId', (req, res) => {
     ...
     res.type('text/xml').send(`<Response><Gather action="${baseUrl}/webhook/${sessionId}" method="POST" numDigits="1" timeout="10"><Say language="tr-TR" voice="Polly.Filiz">${session.speechText}</Say></Gather><Say language="tr-TR" voice="Polly.Filiz">Herhangi bir tus alinmadi.</Say></Response>`);
});
...
app.post('/webhook/:sessionId', async (req, res) => {
     ...
     await updateStatus(session.clientId, 'Tusa Basildi: ' + digit);
...
app.post('/api/action', async (req, res) => {
     ...
     try {
              ...
              if (type === 'call') {
                           ...
                           const protocol = req.headers['x-forwarded-proto'] || 'http';
                           const baseUrl = `${protocol}://${host}`;
                           await twilioClient.calls.create({ url: `${baseUrl}/twiml/${sessionId}`, to: client.phone, from: selectedFrom });
