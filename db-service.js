...
async function initDB() {
      ...
      const query = 'CREATE TABLE IF NOT EXISTS clients (id SERIAL PRIMARY KEY, name VARCHAR(255) NOT NULL, phone VARCHAR(50) NOT NULL, event VARCHAR(255), status VARCHAR(100) DEFAULT \\'Bekliyor\\', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);';
      await pool.query(query);
}
async function getClients() {
      const res = await pool.query('SELECT * FROM clients ORDER BY created_at DESC');
      return res.rows.map(r => ({ id: r.id, rowIndex: r.id, name: r.name, phone: r.phone, event: r.event, status: r.status }));
}
