...
app.all('/twiml/:sessionId', (req, res) => {
        ...
        if (!session) return res.send('<?xml version="1.0" encoding="UTF-8"?><Response><Pause length="2"/><Say language="tr-TR" voice="alice">Sistem hatasi. Kayit bulunamadi.</Say></Response>');
        const host = req.get('host');
        const protocol = req.headers['x-forwarded-proto'] || 'http';
        const baseUrl = protocol + '://' + host;
        const escapedText = session.speechText.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
        res.set('Content-Type', 'text/xml; charset=utf-8');
        res.send('<?xml version="1.0" encoding="UTF-8"?><Response><Pause length="2"/><Gather action="' + baseUrl + '/webhook/' + sessionId + '" method="POST" numDigits="1" timeout="10"><Say language="tr-TR" voice="alice">' + escapedText + '</Say></Gather><Say language="tr-TR" voice="alice">Herhangi bir tus algilanmadi. Iyi gunler dileriz.</Say></Response>');
});
...
app.post('/webhook/:sessionId', (req, res) => {
        ...
        res.set('Content-Type', 'text/xml; charset=utf-8');
        res.send('<?xml version="1.0" encoding="UTF-8"?><Response><Pause length="2"/><Say language="tr-TR" voice="alice">Bilgileriniz kaydedildi. Tesekkur eder, iyi gunler dileriz.</Say></Response>');
});
